...
function Tasks() {
  ...
  const loading = useLoadedPages((state) => {
    if ('Tasks' in state.loadedPages) {
      return state.loadedPages['Tasks'];
    } else {
      return true
    }
  });
      ...
      <div style={loading ? { display: "none"} : null}>
        <div className="relative flex flex-col h-screen overflow-x-hidden pb-[120px]">
          ...
          <div className="relative min-h-[206px] px-[20px] bg-[rgba(117,117,117,0.1)] backdrop-blur-[40px] overflow-hidden flex flex-col justify-end">
            <img className="absolute right-0 bottom-0 z-[-1] w-[50%]" src={tasksImage} alt="" onLoad={() => setLoadedImagesCount(prevState => prevState + 1)} />
