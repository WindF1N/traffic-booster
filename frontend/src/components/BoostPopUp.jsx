...
function BoostPopUp({ setIsOpen, characters, nextCharacterIndex }) {
    ...
    const buyCharacter = (currency, character) => {
        if (currency === 'stars') {
            ...
            if (userFriendlyAddress) {
                const sendTransaction = async (comment) => {
                        ...
                        messages: [
                            {
                                ...
                                amount: toNano(character.price_ton).toString(), //Toncoin in nanotons toNano(0.000000002)
                    ...
                    console.log('Transaction was sent successfully', hashHex);
                    if (response.boc) {
                        setIsOpen(false);
                        addMessage({
                            type: 'success',
                            text: 'Транзакция в обработке блочейном...',
                            name: 'Успех:'
                        })
                    }
                };
                fetch(apiUrl+'/ton_invoice/', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + token
    ...
    return (
        ...
        <div className="boostpopup relative flex flex-col bg-[#282828] rounded-[10px] w-[calc(100%-40px)] h-[73.33%] m-auto overflow-hidden" {...handlers} style={loading ? { display: "none"} : null}>
